document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleError(error) {
        ...
        if (error instanceof SyntaxError) displayError = translations[currentLang].errorJson;
        errorMessage.textContent = displayError;
        errorMessage.classList.remove('hidden');
    }

    function handlePrint() {
        if (!processedMenuData) return alert(translations[currentLang].alertGenerateMenuFirst);
        const lang = translations[currentLang];
        const shoppingList = {};
        processedMenuData.weeklyPlan.forEach(day => Object.values(day.meals).forEach(meal => meal?.ingredients?.forEach(ing => { shoppingList[ing.item.toLowerCase().trim()] = 1; })));
        let shoppingListHtml = `<h3>${lang.printShopListTitle}</h3><ul>${Object.keys(shoppingList).map(item => `<li>${item.charAt(0).toUpperCase() + item.slice(1)}</li>`).join('')}</ul>`;
        if (processedMenuData.added_staples?.length > 0) shoppingListHtml += `<h4>${lang.printStaplesTitle}</h4><ul>${processedMenuData.added_staples.map(item => `<li>${item}</li>`).join('')}</ul>`;
        let planTableHtml = `<h2>${lang.printWeekPlanTitle}</h2><table><thead><tr><th>${lang.printDay}</th><th>${lang.printBreakfast}</th><th>${lang.printLunch}</th><th>${lang.printDinner}</th><th>${lang.printSnack}</th></tr></thead><tbody>${processedMenuData.weeklyPlan.map(day => `<tr><td><strong>${day.day}</strong></td><td>${day.meals.breakfast?.name || ''}</td><td>${day.meals.lunch?.name || ''}</td><td>${day.meals.dinner?.name || ''}</td><td>${day.meals.snack?.name || ''}</td></tr>`).join('')}</tbody></table>`;
        const mealTypeLabels = { breakfast: lang.mealBreakfast, lunch: lang.mealLunch, dinner: lang.mealDinner, snack: lang.mealSnack };
        ...
        processedMenuData.weeklyPlan.forEach(day => {
            ...
            Object.entries(day.meals).forEach(([mealType, meal]) => {
                if (meal?.name) {
                    ...
                    if (meal.ingredients?.length > 0) recipesHtml += '<ul>' + meal.ingredients.map(ing => `<li>${ing.quantity} ${ing.item}</li>`).join('') + '</ul>';
                    if (meal.preparation) recipesHtml += `<p>${meal.preparation.replace(/\n/g, '<br>')}</p>`;
                }
            });
            recipesHtml += `</div>`;
        });
    ...
    function initializeApp() {
        // 1. Attacher immédiatement les écouteurs d'événements à l'interface.
        langSwitcher.addEventListener('change', () => setLanguage(langSwitcher.value));
        ...
        // 2. Définir la langue initiale de l'interface.
        const savedLang = localStorage.getItem('userLanguage');
        const browserLang = navigator.language.split('-')[0];
        const initialLang = savedLang || (translations[browserLang] ? browserLang : 'fr');
        setLanguage(initialLang);

        // 3. Charger les données nutritionnelles en arrière-plan.
        loadCiqualData().then(() => {
            console.log("Application prête et données nutritionnelles chargées.");
        });
    }
